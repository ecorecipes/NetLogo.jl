# ── Participatory Disinformation Model ──────────────────────────
# Source: modelingcommons #6768
# Extensions: nw (network generators), bitstring (rumor tracking)
# Rumor spreading via social network + elite exposure with bitstring ops.
# Stops when ticks >= ticks-to-end or all turtles have heard all rumors.

struct ParticipatoryDisinformationModel <: AbstractBenchmarkModel end

model_name(::ParticipatoryDisinformationModel) = "Participatory Disinformation"
n_ticks(::ParticipatoryDisinformationModel) = 200
tracked_globals(::ParticipatoryDisinformationModel) = [
    "times-heard-from-elites", "times-heard-from-social",
    "times-traded-up", "times-created-rumor"
]

function netlogo_code(::ParticipatoryDisinformationModel)
"""
...
to setup
  ...
  set trade-up-influence 0.25
  set rumor-generation 0.05
  set dissipation-rate 0.1
  set ticks-to-end 200
  set model-rate-1-nth 1
  set heard-rumors-to-generate false
  set influencers-generate-rumors true
  set only-one-generates false
  set only-one-forgets true

...
md"""
# Search Path vignette

This Pluto notebook wraps `eval/models/search_path.jl` in the synthesized NetLogo-style interface used for the vignette rollout.
...
begin
  if !isdefined(Main, :SEARCH_PATH_VIGNETTE_BUNDLE)
    const SEARCH_PATH_VIGNETTE_BUNDLE = Ref{Any}(nothing)
  end
  previous = Main.SEARCH_PATH_VIGNETTE_BUNDLE[]
  previous === nothing || NetLogoVignetteSupport.close_bundle!(previous)
  bundle = NetLogoVignetteSupport.benchmark_backend(Symbol("search_path"); seed=search_path_seed, width=1280, height=980)
  Main.SEARCH_PATH_VIGNETTE_BUNDLE[] = bundle
  bundle
end
...
If you rerun the seed cell above, the notebook tears down the previous local GUI server and starts a fresh Search Path session.
"""

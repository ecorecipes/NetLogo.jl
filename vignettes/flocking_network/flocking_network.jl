...
md"""
# Flocking Network vignette

This Pluto notebook wraps `eval/models/flocking_network.jl` in the synthesized NetLogo-style interface used for the vignette rollout.
...
flocking_network_seed = 1

# ╔═╡ 5e793407-fd26-4942-9e48-3fb75f835986
begin
  if !isdefined(Main, :FLOCKING_NETWORK_VIGNETTE_BUNDLE)
    const FLOCKING_NETWORK_VIGNETTE_BUNDLE = Ref{Any}(nothing)
  end
  previous = Main.FLOCKING_NETWORK_VIGNETTE_BUNDLE[]
  previous === nothing || NetLogoVignetteSupport.close_bundle!(previous)
  bundle = NetLogoVignetteSupport.benchmark_backend(Symbol("flocking_network"); seed=flocking_network_seed, width=1280, height=980)
  Main.FLOCKING_NETWORK_VIGNETTE_BUNDLE[] = bundle
  bundle
end
...
If you rerun the seed cell above, the notebook tears down the previous local GUI server and starts a fresh Flocking Network session.
"""

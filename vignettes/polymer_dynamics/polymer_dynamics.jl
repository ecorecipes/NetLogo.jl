...
md"""
# Polymer Dynamics vignette

This Pluto notebook wraps `eval/models/polymer_dynamics.jl` in the synthesized NetLogo-style interface used for the vignette rollout.
...
polymer_dynamics_seed = 1

# ╔═╡ 5e793407-fd26-4942-9e48-3fb75f835986
begin
  if !isdefined(Main, :POLYMER_DYNAMICS_VIGNETTE_BUNDLE)
    const POLYMER_DYNAMICS_VIGNETTE_BUNDLE = Ref{Any}(nothing)
  end
  previous = Main.POLYMER_DYNAMICS_VIGNETTE_BUNDLE[]
  previous === nothing || NetLogoVignetteSupport.close_bundle!(previous)
  bundle = NetLogoVignetteSupport.benchmark_backend(Symbol("polymer_dynamics"); seed=polymer_dynamics_seed, width=1280, height=980)
  Main.POLYMER_DYNAMICS_VIGNETTE_BUNDLE[] = bundle
  bundle
end
...
If you rerun the seed cell above, the notebook tears down the previous local GUI server and starts a fresh Polymer Dynamics session.
"""

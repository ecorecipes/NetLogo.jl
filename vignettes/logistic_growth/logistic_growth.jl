...
md"""
# Logistic Growth vignette

This Pluto notebook wraps `eval/models/logistic_growth.jl` in the synthesized NetLogo-style interface used for the vignette rollout.
...
logistic_growth_seed = 1

# ╔═╡ 5e793407-fd26-4942-9e48-3fb75f835986
begin
  if !isdefined(Main, :LOGISTIC_GROWTH_VIGNETTE_BUNDLE)
    const LOGISTIC_GROWTH_VIGNETTE_BUNDLE = Ref{Any}(nothing)
  end
  previous = Main.LOGISTIC_GROWTH_VIGNETTE_BUNDLE[]
  previous === nothing || NetLogoVignetteSupport.close_bundle!(previous)
  bundle = NetLogoVignetteSupport.benchmark_backend(Symbol("logistic_growth"); seed=logistic_growth_seed, width=1280, height=980)
  Main.LOGISTIC_GROWTH_VIGNETTE_BUNDLE[] = bundle
  bundle
end
...
If you rerun the seed cell above, the notebook tears down the previous local GUI server and starts a fresh Logistic Growth session.
"""

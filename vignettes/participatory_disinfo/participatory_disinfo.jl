...
This Pluto notebook wraps `eval/models/participatory_disinfo.jl` in the synthesized NetLogo-style interface used for the vignette rollout.
...
participatory_disinfo_seed = 1

# ╔═╡ 5e793407-fd26-4942-9e48-3fb75f835986
begin
  if !isdefined(Main, :PARTICIPATORY_DISINFO_VIGNETTE_BUNDLE)
    const PARTICIPATORY_DISINFO_VIGNETTE_BUNDLE = Ref{Any}(nothing)
  end
  previous = Main.PARTICIPATORY_DISINFO_VIGNETTE_BUNDLE[]
  previous === nothing || NetLogoVignetteSupport.close_bundle!(previous)
  bundle = NetLogoVignetteSupport.benchmark_backend(Symbol("participatory_disinfo"); seed=participatory_disinfo_seed, width=1280, height=980)
  Main.PARTICIPATORY_DISINFO_VIGNETTE_BUNDLE[] = bundle
  bundle
end

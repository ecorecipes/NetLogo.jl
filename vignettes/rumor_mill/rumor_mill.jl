...
This Pluto notebook wraps `eval/models/rumor_mill.jl` in the synthesized NetLogo-style interface used for the vignette rollout.
...
begin
  if !isdefined(Main, :RUMOR_MILL_VIGNETTE_BUNDLE)
    const RUMOR_MILL_VIGNETTE_BUNDLE = Ref{Any}(nothing)
  end
  previous = Main.RUMOR_MILL_VIGNETTE_BUNDLE[]
  previous === nothing || NetLogoVignetteSupport.close_bundle!(previous)
  bundle = NetLogoVignetteSupport.benchmark_backend(Symbol("rumor_mill"); seed=rumor_mill_seed, width=1280, height=980)
  Main.RUMOR_MILL_VIGNETTE_BUNDLE[] = bundle
  bundle
end
...
If you rerun the seed cell above, the notebook tears down the previous local GUI server and starts a fresh Rumor Mill session.
"""

...
md"""
# Language Borders vignette

This Pluto notebook wraps `eval/models/language_borders.jl` in the synthesized NetLogo-style interface used for the vignette rollout.
...
language_borders_seed = 1

# ╔═╡ 5e793407-fd26-4942-9e48-3fb75f835986
begin
  if !isdefined(Main, :LANGUAGE_BORDERS_VIGNETTE_BUNDLE)
    const LANGUAGE_BORDERS_VIGNETTE_BUNDLE = Ref{Any}(nothing)
  end
  previous = Main.LANGUAGE_BORDERS_VIGNETTE_BUNDLE[]
  previous === nothing || NetLogoVignetteSupport.close_bundle!(previous)
  bundle = NetLogoVignetteSupport.benchmark_backend(Symbol("language_borders"); seed=language_borders_seed, width=1280, height=980)
  Main.LANGUAGE_BORDERS_VIGNETTE_BUNDLE[] = bundle
  bundle
end
...
If you rerun the seed cell above, the notebook tears down the previous local GUI server and starts a fresh Language Borders session.
"""

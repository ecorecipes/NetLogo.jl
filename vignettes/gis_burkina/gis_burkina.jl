...
md"""
# GIS Burkina Faso vignette

This Pluto notebook wraps `eval/models/gis_burkina.jl` in the synthesized NetLogo-style interface used for the vignette rollout.
...
begin
  if !isdefined(Main, :GIS_BURKINA_VIGNETTE_BUNDLE)
    const GIS_BURKINA_VIGNETTE_BUNDLE = Ref{Any}(nothing)
  end
  previous = Main.GIS_BURKINA_VIGNETTE_BUNDLE[]
  previous === nothing || NetLogoVignetteSupport.close_bundle!(previous)
  bundle = NetLogoVignetteSupport.benchmark_backend(Symbol("gis_burkina"); seed=gis_burkina_seed, width=1280, height=980)
  Main.GIS_BURKINA_VIGNETTE_BUNDLE[] = bundle
  bundle
end
...
If you rerun the seed cell above, the notebook tears down the previous local GUI server and starts a fresh GIS Burkina Faso session.
"""

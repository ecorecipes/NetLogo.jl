...
md"""
# Recruiting Supporters vignette

This Pluto notebook wraps `eval/models/recruiting_supporters.jl` in the synthesized NetLogo-style interface used for the vignette rollout.
...
recruiting_supporters_seed = 1

# ╔═╡ 5e793407-fd26-4942-9e48-3fb75f835986
begin
  if !isdefined(Main, :RECRUITING_SUPPORTERS_VIGNETTE_BUNDLE)
    const RECRUITING_SUPPORTERS_VIGNETTE_BUNDLE = Ref{Any}(nothing)
  end
  previous = Main.RECRUITING_SUPPORTERS_VIGNETTE_BUNDLE[]
  previous === nothing || NetLogoVignetteSupport.close_bundle!(previous)
  bundle = NetLogoVignetteSupport.benchmark_backend(Symbol("recruiting_supporters"); seed=recruiting_supporters_seed, width=1280, height=980)
  Main.RECRUITING_SUPPORTERS_VIGNETTE_BUNDLE[] = bundle
  bundle
end
...
If you rerun the seed cell above, the notebook tears down the previous local GUI server and starts a fresh Recruiting Supporters session.
"""

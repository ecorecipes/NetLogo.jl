...
This Pluto notebook wraps `eval/models/aids.jl` in the synthesized NetLogo-style interface used for the vignette rollout.
...
begin
  if !isdefined(Main, :AIDS_VIGNETTE_BUNDLE)
    const AIDS_VIGNETTE_BUNDLE = Ref{Any}(nothing)
  end
  previous = Main.AIDS_VIGNETTE_BUNDLE[]
  previous === nothing || NetLogoVignetteSupport.close_bundle!(previous)
  bundle = NetLogoVignetteSupport.benchmark_backend(Symbol("aids"); seed=aids_seed, width=1280, height=980)
  Main.AIDS_VIGNETTE_BUNDLE[] = bundle
  bundle
end
...
If you rerun the seed cell above, the notebook tears down the previous local GUI server and starts a fresh AIDS session.
"""

...
md"""
# Resilient Team vignette

This Pluto notebook wraps `eval/models/resilient_team.jl` in the synthesized NetLogo-style interface used for the vignette rollout.
...
resilient_team_seed = 1

# ╔═╡ 5e793407-fd26-4942-9e48-3fb75f835986
begin
  if !isdefined(Main, :RESILIENT_TEAM_VIGNETTE_BUNDLE)
    const RESILIENT_TEAM_VIGNETTE_BUNDLE = Ref{Any}(nothing)
  end
  previous = Main.RESILIENT_TEAM_VIGNETTE_BUNDLE[]
  previous === nothing || NetLogoVignetteSupport.close_bundle!(previous)
  bundle = NetLogoVignetteSupport.benchmark_backend(Symbol("resilient_team"); seed=resilient_team_seed, width=1280, height=980)
  Main.RESILIENT_TEAM_VIGNETTE_BUNDLE[] = bundle
  bundle
end
...
If you rerun the seed cell above, the notebook tears down the previous local GUI server and starts a fresh Resilient Team session.
"""

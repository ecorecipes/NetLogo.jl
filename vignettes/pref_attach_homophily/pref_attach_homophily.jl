...
md"""
# Pref Attach Homophily vignette

This Pluto notebook wraps `eval/models/pref_attach_homophily.jl` in the synthesized NetLogo-style interface used for the vignette rollout.
...
pref_attach_homophily_seed = 1

# ╔═╡ 5e793407-fd26-4942-9e48-3fb75f835986
begin
  if !isdefined(Main, :PREF_ATTACH_HOMOPHILY_VIGNETTE_BUNDLE)
    const PREF_ATTACH_HOMOPHILY_VIGNETTE_BUNDLE = Ref{Any}(nothing)
  end
  previous = Main.PREF_ATTACH_HOMOPHILY_VIGNETTE_BUNDLE[]
  previous === nothing || NetLogoVignetteSupport.close_bundle!(previous)
  bundle = NetLogoVignetteSupport.benchmark_backend(Symbol("pref_attach_homophily"); seed=pref_attach_homophily_seed, width=1280, height=980)
  Main.PREF_ATTACH_HOMOPHILY_VIGNETTE_BUNDLE[] = bundle
  bundle
end
...
If you rerun the seed cell above, the notebook tears down the previous local GUI server and starts a fresh Pref Attach Homophily session.
"""

...
md"""
# Spatial Attitude vignette

This Pluto notebook wraps `eval/models/spatial_attitude.jl` in the synthesized NetLogo-style interface used for the vignette rollout.
...
spatial_attitude_seed = 1

# ╔═╡ 5e793407-fd26-4942-9e48-3fb75f835986
begin
  if !isdefined(Main, :SPATIAL_ATTITUDE_VIGNETTE_BUNDLE)
    const SPATIAL_ATTITUDE_VIGNETTE_BUNDLE = Ref{Any}(nothing)
  end
  previous = Main.SPATIAL_ATTITUDE_VIGNETTE_BUNDLE[]
  previous === nothing || NetLogoVignetteSupport.close_bundle!(previous)
  bundle = NetLogoVignetteSupport.benchmark_backend(Symbol("spatial_attitude"); seed=spatial_attitude_seed, width=1280, height=980)
  Main.SPATIAL_ATTITUDE_VIGNETTE_BUNDLE[] = bundle
  bundle
end
...
If you rerun the seed cell above, the notebook tears down the previous local GUI server and starts a fresh Spatial Attitude session.
"""

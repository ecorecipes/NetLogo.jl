...
md"""
# Coordination 2 Groups vignette

This Pluto notebook wraps `eval/models/coordination_2groups.jl` in the synthesized NetLogo-style interface used for the vignette rollout.
...
coordination_2groups_seed = 1

# ╔═╡ 5e793407-fd26-4942-9e48-3fb75f835986
begin
  if !isdefined(Main, :COORDINATION_2GROUPS_VIGNETTE_BUNDLE)
    const COORDINATION_2GROUPS_VIGNETTE_BUNDLE = Ref{Any}(nothing)
  end
  previous = Main.COORDINATION_2GROUPS_VIGNETTE_BUNDLE[]
  previous === nothing || NetLogoVignetteSupport.close_bundle!(previous)
  bundle = NetLogoVignetteSupport.benchmark_backend(Symbol("coordination_2groups"); seed=coordination_2groups_seed, width=1280, height=980)
  Main.COORDINATION_2GROUPS_VIGNETTE_BUNDLE[] = bundle
  bundle
end
...
If you rerun the seed cell above, the notebook tears down the previous local GUI server and starts a fresh Coordination 2 Groups session.
"""

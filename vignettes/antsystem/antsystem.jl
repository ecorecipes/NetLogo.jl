...
md"""
# Ant System TSP vignette

This Pluto notebook wraps `eval/models/antsystem.jl` in the synthesized NetLogo-style interface used for the vignette rollout.
...
begin
  if !isdefined(Main, :ANTSYSTEM_VIGNETTE_BUNDLE)
    const ANTSYSTEM_VIGNETTE_BUNDLE = Ref{Any}(nothing)
  end
  previous = Main.ANTSYSTEM_VIGNETTE_BUNDLE[]
  previous === nothing || NetLogoVignetteSupport.close_bundle!(previous)
  bundle = NetLogoVignetteSupport.benchmark_backend(Symbol("antsystem"); seed=antsystem_seed, width=1280, height=980)
  Main.ANTSYSTEM_VIGNETTE_BUNDLE[] = bundle
  bundle
end
...
If you rerun the seed cell above, the notebook tears down the previous local GUI server and starts a fresh Ant System TSP session.
"""

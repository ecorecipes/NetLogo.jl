...
md"""
# Rise of Radicalism vignette

This Pluto notebook wraps `eval/models/rise_of_radicalism.jl` in the synthesized NetLogo-style interface used for the vignette rollout.
...
rise_of_radicalism_seed = 1

# ╔═╡ 5e793407-fd26-4942-9e48-3fb75f835986
begin
  if !isdefined(Main, :RISE_OF_RADICALISM_VIGNETTE_BUNDLE)
    const RISE_OF_RADICALISM_VIGNETTE_BUNDLE = Ref{Any}(nothing)
  end
  previous = Main.RISE_OF_RADICALISM_VIGNETTE_BUNDLE[]
  previous === nothing || NetLogoVignetteSupport.close_bundle!(previous)
  bundle = NetLogoVignetteSupport.benchmark_backend(Symbol("rise_of_radicalism"); seed=rise_of_radicalism_seed, width=1280, height=980)
  Main.RISE_OF_RADICALISM_VIGNETTE_BUNDLE[] = bundle
  bundle
end
...
If you rerun the seed cell above, the notebook tears down the previous local GUI server and starts a fresh Rise of Radicalism session.
"""

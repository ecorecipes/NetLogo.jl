...
md"""
# Info Diffusion vignette

This Pluto notebook wraps `eval/models/info_diffusion.jl` in the synthesized NetLogo-style interface used for the vignette rollout.
...
info_diffusion_seed = 1

# ╔═╡ 5e793407-fd26-4942-9e48-3fb75f835986
begin
  if !isdefined(Main, :INFO_DIFFUSION_VIGNETTE_BUNDLE)
    const INFO_DIFFUSION_VIGNETTE_BUNDLE = Ref{Any}(nothing)
  end
  previous = Main.INFO_DIFFUSION_VIGNETTE_BUNDLE[]
  previous === nothing || NetLogoVignetteSupport.close_bundle!(previous)
  bundle = NetLogoVignetteSupport.benchmark_backend(Symbol("info_diffusion"); seed=info_diffusion_seed, width=1280, height=980)
  Main.INFO_DIFFUSION_VIGNETTE_BUNDLE[] = bundle
  bundle
end
...
If you rerun the seed cell above, the notebook tears down the previous local GUI server and starts a fresh Info Diffusion session.
"""

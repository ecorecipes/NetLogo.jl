...
md"""
# GIS HillClimber vignette

This Pluto notebook wraps `eval/models/gis_hillclimber.jl` in the synthesized NetLogo-style interface used for the vignette rollout.
...
gis_hillclimber_seed = 1

# ╔═╡ 5e793407-fd26-4942-9e48-3fb75f835986
begin
  if !isdefined(Main, :GIS_HILLCLIMBER_VIGNETTE_BUNDLE)
    const GIS_HILLCLIMBER_VIGNETTE_BUNDLE = Ref{Any}(nothing)
  end
  previous = Main.GIS_HILLCLIMBER_VIGNETTE_BUNDLE[]
  previous === nothing || NetLogoVignetteSupport.close_bundle!(previous)
  bundle = NetLogoVignetteSupport.benchmark_backend(Symbol("gis_hillclimber"); seed=gis_hillclimber_seed, width=1280, height=980)
  Main.GIS_HILLCLIMBER_VIGNETTE_BUNDLE[] = bundle
  bundle
end
...
If you rerun the seed cell above, the notebook tears down the previous local GUI server and starts a fresh GIS HillClimber session.
"""

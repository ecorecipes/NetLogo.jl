...
md"""
# Rhino Poaching vignette

This Pluto notebook wraps `eval/models/rhino_poaching.jl` in the synthesized NetLogo-style interface used for the vignette rollout.
...
rhino_poaching_seed = 1

# ╔═╡ 5e793407-fd26-4942-9e48-3fb75f835986
begin
  if !isdefined(Main, :RHINO_POACHING_VIGNETTE_BUNDLE)
    const RHINO_POACHING_VIGNETTE_BUNDLE = Ref{Any}(nothing)
  end
  previous = Main.RHINO_POACHING_VIGNETTE_BUNDLE[]
  previous === nothing || NetLogoVignetteSupport.close_bundle!(previous)
  bundle = NetLogoVignetteSupport.benchmark_backend(Symbol("rhino_poaching"); seed=rhino_poaching_seed, width=1280, height=980)
  Main.RHINO_POACHING_VIGNETTE_BUNDLE[] = bundle
  bundle
end
...
If you rerun the seed cell above, the notebook tears down the previous local GUI server and starts a fresh Rhino Poaching session.
"""

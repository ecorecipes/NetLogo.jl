...
md"""
# Contagion NW vignette

This Pluto notebook wraps `eval/models/contagion_nw.jl` in the synthesized NetLogo-style interface used for the vignette rollout.
...
begin
  if !isdefined(Main, :CONTAGION_NW_VIGNETTE_BUNDLE)
    const CONTAGION_NW_VIGNETTE_BUNDLE = Ref{Any}(nothing)
  end
  previous = Main.CONTAGION_NW_VIGNETTE_BUNDLE[]
  previous === nothing || NetLogoVignetteSupport.close_bundle!(previous)
  bundle = NetLogoVignetteSupport.benchmark_backend(Symbol("contagion_nw"); seed=contagion_nw_seed, width=1280, height=980)
  Main.CONTAGION_NW_VIGNETTE_BUNDLE[] = bundle
  bundle
end
...
If you rerun the seed cell above, the notebook tears down the previous local GUI server and starts a fresh Contagion NW session.
"""

...
md"""
# Baby Sitting Coop vignette

This Pluto notebook wraps `eval/models/babysitting_coop.jl` in the synthesized NetLogo-style interface used for the vignette rollout.
...
babysitting_coop_seed = 1

# ╔═╡ 5e793407-fd26-4942-9e48-3fb75f835986
begin
  if !isdefined(Main, :BABYSITTING_COOP_VIGNETTE_BUNDLE)
    const BABYSITTING_COOP_VIGNETTE_BUNDLE = Ref{Any}(nothing)
  end
  previous = Main.BABYSITTING_COOP_VIGNETTE_BUNDLE[]
  previous === nothing || NetLogoVignetteSupport.close_bundle!(previous)
  bundle = NetLogoVignetteSupport.benchmark_backend(Symbol("babysitting_coop"); seed=babysitting_coop_seed, width=1280, height=980)
  Main.BABYSITTING_COOP_VIGNETTE_BUNDLE[] = bundle
  bundle
end
...
If you rerun the seed cell above, the notebook tears down the previous local GUI server and starts a fresh Baby Sitting Coop session.
"""

...
This Pluto notebook wraps `eval/models/pd_simple.jl` in the synthesized NetLogo-style interface used for the vignette rollout.
...
begin
  if !isdefined(Main, :PD_SIMPLE_VIGNETTE_BUNDLE)
    const PD_SIMPLE_VIGNETTE_BUNDLE = Ref{Any}(nothing)
  end
  previous = Main.PD_SIMPLE_VIGNETTE_BUNDLE[]
  previous === nothing || NetLogoVignetteSupport.close_bundle!(previous)
  bundle = NetLogoVignetteSupport.benchmark_backend(Symbol("pd_simple"); seed=pd_simple_seed, width=1280, height=980)
  Main.PD_SIMPLE_VIGNETTE_BUNDLE[] = bundle
  bundle
end
...
If you rerun the seed cell above, the notebook tears down the previous local GUI server and starts a fresh PD Simple session.
"""

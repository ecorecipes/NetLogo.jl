...
md"""
# Bullwhip Effect vignette

This Pluto notebook wraps `eval/models/bullwhip_effect.jl` in the synthesized NetLogo-style interface used for the vignette rollout.
...
bullwhip_effect_seed = 1

# ╔═╡ 5e793407-fd26-4942-9e48-3fb75f835986
begin
  if !isdefined(Main, :BULLWHIP_EFFECT_VIGNETTE_BUNDLE)
    const BULLWHIP_EFFECT_VIGNETTE_BUNDLE = Ref{Any}(nothing)
  end
  previous = Main.BULLWHIP_EFFECT_VIGNETTE_BUNDLE[]
  previous === nothing || NetLogoVignetteSupport.close_bundle!(previous)
  bundle = NetLogoVignetteSupport.benchmark_backend(Symbol("bullwhip_effect"); seed=bullwhip_effect_seed, width=1280, height=980)
  Main.BULLWHIP_EFFECT_VIGNETTE_BUNDLE[] = bundle
  bundle
end
...
If you rerun the seed cell above, the notebook tears down the previous local GUI server and starts a fresh Bullwhip Effect session.
"""

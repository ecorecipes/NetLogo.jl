...
md"""
# Bank Reserves vignette

This Pluto notebook wraps `eval/models/bank_reserves.jl` in the synthesized NetLogo-style interface used for the vignette rollout.
...
bank_reserves_seed = 1

# ╔═╡ 5e793407-fd26-4942-9e48-3fb75f835986
begin
  if !isdefined(Main, :BANK_RESERVES_VIGNETTE_BUNDLE)
    const BANK_RESERVES_VIGNETTE_BUNDLE = Ref{Any}(nothing)
  end
  previous = Main.BANK_RESERVES_VIGNETTE_BUNDLE[]
  previous === nothing || NetLogoVignetteSupport.close_bundle!(previous)
  bundle = NetLogoVignetteSupport.benchmark_backend(Symbol("bank_reserves"); seed=bank_reserves_seed, width=1280, height=980)
  Main.BANK_RESERVES_VIGNETTE_BUNDLE[] = bundle
  bundle
end
...
If you rerun the seed cell above, the notebook tears down the previous local GUI server and starts a fresh Bank Reserves session.
"""

...
md"""
# Being Green vignette

This Pluto notebook wraps `eval/models/being_green.jl` in the synthesized NetLogo-style interface used for the vignette rollout.
...
begin
  if !isdefined(Main, :BEING_GREEN_VIGNETTE_BUNDLE)
    const BEING_GREEN_VIGNETTE_BUNDLE = Ref{Any}(nothing)
  end
  previous = Main.BEING_GREEN_VIGNETTE_BUNDLE[]
  previous === nothing || NetLogoVignetteSupport.close_bundle!(previous)
  bundle = NetLogoVignetteSupport.benchmark_backend(Symbol("being_green"); seed=being_green_seed, width=1280, height=980)
  Main.BEING_GREEN_VIGNETTE_BUNDLE[] = bundle
  bundle
end
...
If you rerun the seed cell above, the notebook tears down the previous local GUI server and starts a fresh Being Green session.
"""

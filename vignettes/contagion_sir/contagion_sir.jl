...
md"""
# Contagion SIR vignette

This Pluto notebook wraps `eval/models/contagion_sir.jl` in the synthesized NetLogo-style interface used for the vignette rollout.
...
contagion_sir_seed = 1

# ╔═╡ 5e793407-fd26-4942-9e48-3fb75f835986
begin
  if !isdefined(Main, :CONTAGION_SIR_VIGNETTE_BUNDLE)
    const CONTAGION_SIR_VIGNETTE_BUNDLE = Ref{Any}(nothing)
  end
  previous = Main.CONTAGION_SIR_VIGNETTE_BUNDLE[]
  previous === nothing || NetLogoVignetteSupport.close_bundle!(previous)
  bundle = NetLogoVignetteSupport.benchmark_backend(Symbol("contagion_sir"); seed=contagion_sir_seed, width=1280, height=980)
  Main.CONTAGION_SIR_VIGNETTE_BUNDLE[] = bundle
  bundle
end
...
If you rerun the seed cell above, the notebook tears down the previous local GUI server and starts a fresh Contagion SIR session.
"""

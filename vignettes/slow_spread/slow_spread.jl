...
md"""
# Slow Spread Env Action vignette

This Pluto notebook wraps `eval/models/slow_spread.jl` in the synthesized NetLogo-style interface used for the vignette rollout.
...
begin
  if !isdefined(Main, :SLOW_SPREAD_VIGNETTE_BUNDLE)
    const SLOW_SPREAD_VIGNETTE_BUNDLE = Ref{Any}(nothing)
  end
  previous = Main.SLOW_SPREAD_VIGNETTE_BUNDLE[]
  previous === nothing || NetLogoVignetteSupport.close_bundle!(previous)
  bundle = NetLogoVignetteSupport.benchmark_backend(Symbol("slow_spread"); seed=slow_spread_seed, width=1280, height=980)
  Main.SLOW_SPREAD_VIGNETTE_BUNDLE[] = bundle
  bundle
end
...
If you rerun the seed cell above, the notebook tears down the previous local GUI server and starts a fresh Slow Spread Env Action session.
"""

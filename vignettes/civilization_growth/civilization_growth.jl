...
md"""
# Civilization Growth vignette

This Pluto notebook wraps `eval/models/civilization_growth.jl` in the synthesized NetLogo-style interface used for the vignette rollout.
...
civilization_growth_seed = 1

# ╔═╡ 5e793407-fd26-4942-9e48-3fb75f835986
begin
  if !isdefined(Main, :CIVILIZATION_GROWTH_VIGNETTE_BUNDLE)
    const CIVILIZATION_GROWTH_VIGNETTE_BUNDLE = Ref{Any}(nothing)
  end
  previous = Main.CIVILIZATION_GROWTH_VIGNETTE_BUNDLE[]
  previous === nothing || NetLogoVignetteSupport.close_bundle!(previous)
  bundle = NetLogoVignetteSupport.benchmark_backend(Symbol("civilization_growth"); seed=civilization_growth_seed, width=1280, height=980)
  Main.CIVILIZATION_GROWTH_VIGNETTE_BUNDLE[] = bundle
  bundle
end
...
If you rerun the seed cell above, the notebook tears down the previous local GUI server and starts a fresh Civilization Growth session.
"""

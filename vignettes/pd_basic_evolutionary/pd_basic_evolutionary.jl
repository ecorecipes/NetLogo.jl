...
md"""
# PD Basic Evolutionary vignette

This Pluto notebook wraps `eval/models/pd_basic_evolutionary.jl` in the synthesized NetLogo-style interface used for the vignette rollout.
...
pd_basic_evolutionary_seed = 1

# ╔═╡ 5e793407-fd26-4942-9e48-3fb75f835986
begin
  if !isdefined(Main, :PD_BASIC_EVOLUTIONARY_VIGNETTE_BUNDLE)
    const PD_BASIC_EVOLUTIONARY_VIGNETTE_BUNDLE = Ref{Any}(nothing)
  end
  previous = Main.PD_BASIC_EVOLUTIONARY_VIGNETTE_BUNDLE[]
  previous === nothing || NetLogoVignetteSupport.close_bundle!(previous)
  bundle = NetLogoVignetteSupport.benchmark_backend(Symbol("pd_basic_evolutionary"); seed=pd_basic_evolutionary_seed, width=1280, height=980)
  Main.PD_BASIC_EVOLUTIONARY_VIGNETTE_BUNDLE[] = bundle
  bundle
end
...
If you rerun the seed cell above, the notebook tears down the previous local GUI server and starts a fresh PD Basic Evolutionary session.
"""

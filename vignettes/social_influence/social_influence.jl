...
md"""
# Social Influence vignette

This Pluto notebook wraps `eval/models/social_influence.jl` in the synthesized NetLogo-style interface used for the vignette rollout.
...
social_influence_seed = 1

# ╔═╡ 5e793407-fd26-4942-9e48-3fb75f835986
begin
  if !isdefined(Main, :SOCIAL_INFLUENCE_VIGNETTE_BUNDLE)
    const SOCIAL_INFLUENCE_VIGNETTE_BUNDLE = Ref{Any}(nothing)
  end
  previous = Main.SOCIAL_INFLUENCE_VIGNETTE_BUNDLE[]
  previous === nothing || NetLogoVignetteSupport.close_bundle!(previous)
  bundle = NetLogoVignetteSupport.benchmark_backend(Symbol("social_influence"); seed=social_influence_seed, width=1280, height=980)
  Main.SOCIAL_INFLUENCE_VIGNETTE_BUNDLE[] = bundle
  bundle
end
...
If you rerun the seed cell above, the notebook tears down the previous local GUI server and starts a fresh Social Influence session.
"""

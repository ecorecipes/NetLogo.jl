...
md"""
# Voronoi Voter vignette

This Pluto notebook wraps `eval/models/voronoi_voter.jl` in the synthesized NetLogo-style interface used for the vignette rollout.
...
voronoi_voter_seed = 1

# ╔═╡ 5e793407-fd26-4942-9e48-3fb75f835986
begin
  if !isdefined(Main, :VORONOI_VOTER_VIGNETTE_BUNDLE)
    const VORONOI_VOTER_VIGNETTE_BUNDLE = Ref{Any}(nothing)
  end
  previous = Main.VORONOI_VOTER_VIGNETTE_BUNDLE[]
  previous === nothing || NetLogoVignetteSupport.close_bundle!(previous)
  bundle = NetLogoVignetteSupport.benchmark_backend(Symbol("voronoi_voter"); seed=voronoi_voter_seed, width=1280, height=980)
  Main.VORONOI_VOTER_VIGNETTE_BUNDLE[] = bundle
  bundle
end
...
If you rerun the seed cell above, the notebook tears down the previous local GUI server and starts a fresh Voronoi Voter session.
"""

...
md"""
# Sugarscape Culture vignette

This Pluto notebook wraps `eval/models/sugarscape_culture.jl` in the synthesized NetLogo-style interface used for the vignette rollout.
...
sugarscape_culture_seed = 1

# ╔═╡ 5e793407-fd26-4942-9e48-3fb75f835986
begin
  if !isdefined(Main, :SUGARSCAPE_CULTURE_VIGNETTE_BUNDLE)
    const SUGARSCAPE_CULTURE_VIGNETTE_BUNDLE = Ref{Any}(nothing)
  end
  previous = Main.SUGARSCAPE_CULTURE_VIGNETTE_BUNDLE[]
  previous === nothing || NetLogoVignetteSupport.close_bundle!(previous)
  bundle = NetLogoVignetteSupport.benchmark_backend(Symbol("sugarscape_culture"); seed=sugarscape_culture_seed, width=1280, height=980)
  Main.SUGARSCAPE_CULTURE_VIGNETTE_BUNDLE[] = bundle
  bundle
end
...
If you rerun the seed cell above, the notebook tears down the previous local GUI server and starts a fresh Sugarscape Culture session.
"""

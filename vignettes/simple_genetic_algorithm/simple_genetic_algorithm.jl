...
md"""
# Simple Genetic Algorithm vignette

This Pluto notebook wraps `eval/models/simple_genetic_algorithm.jl` in the synthesized NetLogo-style interface used for the vignette rollout.
...
simple_genetic_algorithm_seed = 1

# ╔═╡ 5e793407-fd26-4942-9e48-3fb75f835986
begin
  if !isdefined(Main, :SIMPLE_GENETIC_ALGORITHM_VIGNETTE_BUNDLE)
    const SIMPLE_GENETIC_ALGORITHM_VIGNETTE_BUNDLE = Ref{Any}(nothing)
  end
  previous = Main.SIMPLE_GENETIC_ALGORITHM_VIGNETTE_BUNDLE[]
  previous === nothing || NetLogoVignetteSupport.close_bundle!(previous)
  bundle = NetLogoVignetteSupport.benchmark_backend(Symbol("simple_genetic_algorithm"); seed=simple_genetic_algorithm_seed, width=1280, height=980)
  Main.SIMPLE_GENETIC_ALGORITHM_VIGNETTE_BUNDLE[] = bundle
  bundle
end
...
If you rerun the seed cell above, the notebook tears down the previous local GUI server and starts a fresh Simple Genetic Algorithm session.
"""

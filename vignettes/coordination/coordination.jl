...
md"""
# Coordination vignette

This Pluto notebook wraps `eval/models/coordination.jl` in the synthesized NetLogo-style interface used for the vignette rollout.
...
begin
  if !isdefined(Main, :COORDINATION_VIGNETTE_BUNDLE)
    const COORDINATION_VIGNETTE_BUNDLE = Ref{Any}(nothing)
  end
  previous = Main.COORDINATION_VIGNETTE_BUNDLE[]
  previous === nothing || NetLogoVignetteSupport.close_bundle!(previous)
  bundle = NetLogoVignetteSupport.benchmark_backend(Symbol("coordination"); seed=coordination_seed, width=1280, height=980)
  Main.COORDINATION_VIGNETTE_BUNDLE[] = bundle
  bundle
end
...
If you rerun the seed cell above, the notebook tears down the previous local GUI server and starts a fresh Coordination session.
"""

...
md"""
# Opinion Dynamics BC vignette

This Pluto notebook wraps `eval/models/opinion_bc.jl` in the synthesized NetLogo-style interface used for the vignette rollout.
...
begin
  if !isdefined(Main, :OPINION_BC_VIGNETTE_BUNDLE)
    const OPINION_BC_VIGNETTE_BUNDLE = Ref{Any}(nothing)
  end
  previous = Main.OPINION_BC_VIGNETTE_BUNDLE[]
  previous === nothing || NetLogoVignetteSupport.close_bundle!(previous)
  bundle = NetLogoVignetteSupport.benchmark_backend(Symbol("opinion_bc"); seed=opinion_bc_seed, width=1280, height=980)
  Main.OPINION_BC_VIGNETTE_BUNDLE[] = bundle
  bundle
end
...
If you rerun the seed cell above, the notebook tears down the previous local GUI server and starts a fresh Opinion Dynamics BC session.
"""

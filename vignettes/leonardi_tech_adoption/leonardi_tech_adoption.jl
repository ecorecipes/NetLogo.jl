...
md"""
# Leonardi Tech Adoption vignette

This Pluto notebook wraps `eval/models/leonardi_tech_adoption.jl` in the synthesized NetLogo-style interface used for the vignette rollout.
...
leonardi_tech_adoption_seed = 1

# ╔═╡ 5e793407-fd26-4942-9e48-3fb75f835986
begin
  if !isdefined(Main, :LEONARDI_TECH_ADOPTION_VIGNETTE_BUNDLE)
    const LEONARDI_TECH_ADOPTION_VIGNETTE_BUNDLE = Ref{Any}(nothing)
  end
  previous = Main.LEONARDI_TECH_ADOPTION_VIGNETTE_BUNDLE[]
  previous === nothing || NetLogoVignetteSupport.close_bundle!(previous)
  bundle = NetLogoVignetteSupport.benchmark_backend(Symbol("leonardi_tech_adoption"); seed=leonardi_tech_adoption_seed, width=1280, height=980)
  Main.LEONARDI_TECH_ADOPTION_VIGNETTE_BUNDLE[] = bundle
  bundle
end
...
If you rerun the seed cell above, the notebook tears down the previous local GUI server and starts a fresh Leonardi Tech Adoption session.
"""

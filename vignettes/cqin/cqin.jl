...
md"""
# Quality Improvement Network vignette

This Pluto notebook wraps `eval/models/cqin.jl` in the synthesized NetLogo-style interface used for the vignette rollout.
...
begin
  if !isdefined(Main, :CQIN_VIGNETTE_BUNDLE)
    const CQIN_VIGNETTE_BUNDLE = Ref{Any}(nothing)
  end
  previous = Main.CQIN_VIGNETTE_BUNDLE[]
  previous === nothing || NetLogoVignetteSupport.close_bundle!(previous)
  bundle = NetLogoVignetteSupport.benchmark_backend(Symbol("cqin"); seed=cqin_seed, width=1280, height=980)
  Main.CQIN_VIGNETTE_BUNDLE[] = bundle
  bundle
end
...
If you rerun the seed cell above, the notebook tears down the previous local GUI server and starts a fresh Quality Improvement Network session.
"""

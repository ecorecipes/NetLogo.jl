...
md"""
# Modsoc Segregation vignette

This Pluto notebook wraps `eval/models/modsoc_segregation.jl` in the synthesized NetLogo-style interface used for the vignette rollout.
...
modsoc_segregation_seed = 1

# ╔═╡ 5e793407-fd26-4942-9e48-3fb75f835986
begin
  if !isdefined(Main, :MODSOC_SEGREGATION_VIGNETTE_BUNDLE)
    const MODSOC_SEGREGATION_VIGNETTE_BUNDLE = Ref{Any}(nothing)
  end
  previous = Main.MODSOC_SEGREGATION_VIGNETTE_BUNDLE[]
  previous === nothing || NetLogoVignetteSupport.close_bundle!(previous)
  bundle = NetLogoVignetteSupport.benchmark_backend(Symbol("modsoc_segregation"); seed=modsoc_segregation_seed, width=1280, height=980)
  Main.MODSOC_SEGREGATION_VIGNETTE_BUNDLE[] = bundle
  bundle
end
...
If you rerun the seed cell above, the notebook tears down the previous local GUI server and starts a fresh Modsoc Segregation session.
"""

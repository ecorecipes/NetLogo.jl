...
md"""
# Unit Cohesion vignette

This Pluto notebook wraps `eval/models/unit_cohesion.jl` in the synthesized NetLogo-style interface used for the vignette rollout.
...
unit_cohesion_seed = 1

# ╔═╡ 5e793407-fd26-4942-9e48-3fb75f835986
begin
  if !isdefined(Main, :UNIT_COHESION_VIGNETTE_BUNDLE)
    const UNIT_COHESION_VIGNETTE_BUNDLE = Ref{Any}(nothing)
  end
  previous = Main.UNIT_COHESION_VIGNETTE_BUNDLE[]
  previous === nothing || NetLogoVignetteSupport.close_bundle!(previous)
  bundle = NetLogoVignetteSupport.benchmark_backend(Symbol("unit_cohesion"); seed=unit_cohesion_seed, width=1280, height=980)
  Main.UNIT_COHESION_VIGNETTE_BUNDLE[] = bundle
  bundle
end
...
If you rerun the seed cell above, the notebook tears down the previous local GUI server and starts a fresh Unit Cohesion session.
"""

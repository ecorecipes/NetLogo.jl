...
md"""
# Minority Belief Spread vignette

This Pluto notebook wraps `eval/models/minority_belief.jl` in the synthesized NetLogo-style interface used for the vignette rollout.
...
minority_belief_seed = 1

# ╔═╡ 5e793407-fd26-4942-9e48-3fb75f835986
begin
  if !isdefined(Main, :MINORITY_BELIEF_VIGNETTE_BUNDLE)
    const MINORITY_BELIEF_VIGNETTE_BUNDLE = Ref{Any}(nothing)
  end
  previous = Main.MINORITY_BELIEF_VIGNETTE_BUNDLE[]
  previous === nothing || NetLogoVignetteSupport.close_bundle!(previous)
  bundle = NetLogoVignetteSupport.benchmark_backend(Symbol("minority_belief"); seed=minority_belief_seed, width=1280, height=980)
  Main.MINORITY_BELIEF_VIGNETTE_BUNDLE[] = bundle
  bundle
end
...
If you rerun the seed cell above, the notebook tears down the previous local GUI server and starts a fresh Minority Belief Spread session.
"""

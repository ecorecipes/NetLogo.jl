...
md"""
# Rabbits Grass Weeds vignette

This Pluto notebook wraps `eval/models/rabbits_grass_weeds.jl` in the synthesized NetLogo-style interface used for the vignette rollout.
...
rabbits_grass_weeds_seed = 1

# ╔═╡ 5e793407-fd26-4942-9e48-3fb75f835986
begin
  if !isdefined(Main, :RABBITS_GRASS_WEEDS_VIGNETTE_BUNDLE)
    const RABBITS_GRASS_WEEDS_VIGNETTE_BUNDLE = Ref{Any}(nothing)
  end
  previous = Main.RABBITS_GRASS_WEEDS_VIGNETTE_BUNDLE[]
  previous === nothing || NetLogoVignetteSupport.close_bundle!(previous)
  bundle = NetLogoVignetteSupport.benchmark_backend(Symbol("rabbits_grass_weeds"); seed=rabbits_grass_weeds_seed, width=1280, height=980)
  Main.RABBITS_GRASS_WEEDS_VIGNETTE_BUNDLE[] = bundle
  bundle
end
...
If you rerun the seed cell above, the notebook tears down the previous local GUI server and starts a fresh Rabbits Grass Weeds session.
"""

...
md"""
# Competition Learning vignette

This Pluto notebook wraps `eval/models/competition_learning.jl` in the synthesized NetLogo-style interface used for the vignette rollout.
...
competition_learning_seed = 1

# ╔═╡ 5e793407-fd26-4942-9e48-3fb75f835986
begin
  if !isdefined(Main, :COMPETITION_LEARNING_VIGNETTE_BUNDLE)
    const COMPETITION_LEARNING_VIGNETTE_BUNDLE = Ref{Any}(nothing)
  end
  previous = Main.COMPETITION_LEARNING_VIGNETTE_BUNDLE[]
  previous === nothing || NetLogoVignetteSupport.close_bundle!(previous)
  bundle = NetLogoVignetteSupport.benchmark_backend(Symbol("competition_learning"); seed=competition_learning_seed, width=1280, height=980)
  Main.COMPETITION_LEARNING_VIGNETTE_BUNDLE[] = bundle
  bundle
end
...
If you rerun the seed cell above, the notebook tears down the previous local GUI server and starts a fresh Competition Learning session.
"""

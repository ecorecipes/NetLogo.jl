...
md"""
# Social Network Diffusion vignette

This Pluto notebook wraps `eval/models/social_network_diffusion.jl` in the synthesized NetLogo-style interface used for the vignette rollout.
...
social_network_diffusion_seed = 1

# ╔═╡ 5e793407-fd26-4942-9e48-3fb75f835986
begin
  if !isdefined(Main, :SOCIAL_NETWORK_DIFFUSION_VIGNETTE_BUNDLE)
    const SOCIAL_NETWORK_DIFFUSION_VIGNETTE_BUNDLE = Ref{Any}(nothing)
  end
  previous = Main.SOCIAL_NETWORK_DIFFUSION_VIGNETTE_BUNDLE[]
  previous === nothing || NetLogoVignetteSupport.close_bundle!(previous)
  bundle = NetLogoVignetteSupport.benchmark_backend(Symbol("social_network_diffusion"); seed=social_network_diffusion_seed, width=1280, height=980)
  Main.SOCIAL_NETWORK_DIFFUSION_VIGNETTE_BUNDLE[] = bundle
  bundle
end
...
If you rerun the seed cell above, the notebook tears down the previous local GUI server and starts a fresh Social Network Diffusion session.
"""

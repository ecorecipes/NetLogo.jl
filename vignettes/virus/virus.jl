...
md"""
# Virus on Network vignette

This Pluto notebook wraps `eval/models/virus.jl` in the synthesized NetLogo-style interface used for the vignette rollout.
...
begin
  if !isdefined(Main, :VIRUS_VIGNETTE_BUNDLE)
    const VIRUS_VIGNETTE_BUNDLE = Ref{Any}(nothing)
  end
  previous = Main.VIRUS_VIGNETTE_BUNDLE[]
  previous === nothing || NetLogoVignetteSupport.close_bundle!(previous)
  bundle = NetLogoVignetteSupport.benchmark_backend(Symbol("virus"); seed=virus_seed, width=1280, height=980)
  Main.VIRUS_VIGNETTE_BUNDLE[] = bundle
  bundle
end
...
If you rerun the seed cell above, the notebook tears down the previous local GUI server and starts a fresh Virus on Network session.
"""

...
md"""
# EvolutionOfNorms vignette

This Pluto notebook wraps `eval/models/evolution_of_norms.jl` in the synthesized NetLogo-style interface used for the vignette rollout.
...
evolution_of_norms_seed = 1

# ╔═╡ 5e793407-fd26-4942-9e48-3fb75f835986
begin
  if !isdefined(Main, :EVOLUTION_OF_NORMS_VIGNETTE_BUNDLE)
    const EVOLUTION_OF_NORMS_VIGNETTE_BUNDLE = Ref{Any}(nothing)
  end
  previous = Main.EVOLUTION_OF_NORMS_VIGNETTE_BUNDLE[]
  previous === nothing || NetLogoVignetteSupport.close_bundle!(previous)
  bundle = NetLogoVignetteSupport.benchmark_backend(Symbol("evolution_of_norms"); seed=evolution_of_norms_seed, width=1280, height=980)
  Main.EVOLUTION_OF_NORMS_VIGNETTE_BUNDLE[] = bundle
  bundle
end
...
If you rerun the seed cell above, the notebook tears down the previous local GUI server and starts a fresh EvolutionOfNorms session.
"""

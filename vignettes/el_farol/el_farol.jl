...
This Pluto notebook wraps `eval/models/el_farol.jl` in the synthesized NetLogo-style interface used for the vignette rollout.
...
begin
  if !isdefined(Main, :EL_FAROL_VIGNETTE_BUNDLE)
    const EL_FAROL_VIGNETTE_BUNDLE = Ref{Any}(nothing)
  end
  previous = Main.EL_FAROL_VIGNETTE_BUNDLE[]
  previous === nothing || NetLogoVignetteSupport.close_bundle!(previous)
  bundle = NetLogoVignetteSupport.benchmark_backend(Symbol("el_farol"); seed=el_farol_seed, width=1280, height=980)
  Main.EL_FAROL_VIGNETTE_BUNDLE[] = bundle
  bundle
end
...
If you rerun the seed cell above, the notebook tears down the previous local GUI server and starts a fresh El Farol session.
"""

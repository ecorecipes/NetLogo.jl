...
md"""
# Mailmen Delivery vignette

This Pluto notebook wraps `eval/models/mailmen.jl` in the synthesized NetLogo-style interface used for the vignette rollout.
...
begin
  if !isdefined(Main, :MAILMEN_VIGNETTE_BUNDLE)
    const MAILMEN_VIGNETTE_BUNDLE = Ref{Any}(nothing)
  end
  previous = Main.MAILMEN_VIGNETTE_BUNDLE[]
  previous === nothing || NetLogoVignetteSupport.close_bundle!(previous)
  bundle = NetLogoVignetteSupport.benchmark_backend(Symbol("mailmen"); seed=mailmen_seed, width=1280, height=980)
  Main.MAILMEN_VIGNETTE_BUNDLE[] = bundle
  bundle
end
...
If you rerun the seed cell above, the notebook tears down the previous local GUI server and starts a fresh Mailmen Delivery session.
"""

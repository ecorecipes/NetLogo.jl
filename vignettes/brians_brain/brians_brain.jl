...
md"""
# Brian's Brain vignette

This Pluto notebook wraps `eval/models/brians_brain.jl` in the synthesized NetLogo-style interface used for the vignette rollout.
...
begin
  if !isdefined(Main, :BRIANS_BRAIN_VIGNETTE_BUNDLE)
    const BRIANS_BRAIN_VIGNETTE_BUNDLE = Ref{Any}(nothing)
  end
  previous = Main.BRIANS_BRAIN_VIGNETTE_BUNDLE[]
  previous === nothing || NetLogoVignetteSupport.close_bundle!(previous)
  bundle = NetLogoVignetteSupport.benchmark_backend(Symbol("brians_brain"); seed=brians_brain_seed, width=1280, height=980)
  Main.BRIANS_BRAIN_VIGNETTE_BUNDLE[] = bundle
  bundle
end
...
If you rerun the seed cell above, the notebook tears down the previous local GUI server and starts a fresh Brian's Brain session.
"""

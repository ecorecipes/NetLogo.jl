...
md"""
# Spread of Fake News vignette

This Pluto notebook wraps `eval/models/fake_news.jl` in the synthesized NetLogo-style interface used for the vignette rollout.
...
begin
  if !isdefined(Main, :FAKE_NEWS_VIGNETTE_BUNDLE)
    const FAKE_NEWS_VIGNETTE_BUNDLE = Ref{Any}(nothing)
  end
  previous = Main.FAKE_NEWS_VIGNETTE_BUNDLE[]
  previous === nothing || NetLogoVignetteSupport.close_bundle!(previous)
  bundle = NetLogoVignetteSupport.benchmark_backend(Symbol("fake_news"); seed=fake_news_seed, width=1280, height=980)
  Main.FAKE_NEWS_VIGNETTE_BUNDLE[] = bundle
  bundle
end
...
If you rerun the seed cell above, the notebook tears down the previous local GUI server and starts a fresh Spread of Fake News session.
"""

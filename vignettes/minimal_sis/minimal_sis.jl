...
md"""
# Minimal SIS vignette

This Pluto notebook wraps `eval/models/minimal_sis.jl` in the synthesized NetLogo-style interface used for the vignette rollout.
...
begin
  if !isdefined(Main, :MINIMAL_SIS_VIGNETTE_BUNDLE)
    const MINIMAL_SIS_VIGNETTE_BUNDLE = Ref{Any}(nothing)
  end
  previous = Main.MINIMAL_SIS_VIGNETTE_BUNDLE[]
  previous === nothing || NetLogoVignetteSupport.close_bundle!(previous)
  bundle = NetLogoVignetteSupport.benchmark_backend(Symbol("minimal_sis"); seed=minimal_sis_seed, width=1280, height=980)
  Main.MINIMAL_SIS_VIGNETTE_BUNDLE[] = bundle
  bundle
end
...
If you rerun the seed cell above, the notebook tears down the previous local GUI server and starts a fresh Minimal SIS session.
"""

...
This Pluto notebook wraps `eval/models/drug_use.jl` in the synthesized NetLogo-style interface used for the vignette rollout.
...
begin
  if !isdefined(Main, :DRUG_USE_VIGNETTE_BUNDLE)
    const DRUG_USE_VIGNETTE_BUNDLE = Ref{Any}(nothing)
  end
  previous = Main.DRUG_USE_VIGNETTE_BUNDLE[]
  previous === nothing || NetLogoVignetteSupport.close_bundle!(previous)
  bundle = NetLogoVignetteSupport.benchmark_backend(Symbol("drug_use"); seed=drug_use_seed, width=1280, height=980)
  Main.DRUG_USE_VIGNETTE_BUNDLE[] = bundle
  bundle
end
...
If you rerun the seed cell above, the notebook tears down the previous local GUI server and starts a fresh Drug Use session.
"""

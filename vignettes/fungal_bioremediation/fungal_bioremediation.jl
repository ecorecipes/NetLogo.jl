...
md"""
# Fungal Bioremediation vignette

This Pluto notebook wraps `eval/models/fungal_bioremediation.jl` in the synthesized NetLogo-style interface used for the vignette rollout.
...
fungal_bioremediation_seed = 1

# ╔═╡ 5e793407-fd26-4942-9e48-3fb75f835986
begin
  if !isdefined(Main, :FUNGAL_BIOREMEDIATION_VIGNETTE_BUNDLE)
    const FUNGAL_BIOREMEDIATION_VIGNETTE_BUNDLE = Ref{Any}(nothing)
  end
  previous = Main.FUNGAL_BIOREMEDIATION_VIGNETTE_BUNDLE[]
  previous === nothing || NetLogoVignetteSupport.close_bundle!(previous)
  bundle = NetLogoVignetteSupport.benchmark_backend(Symbol("fungal_bioremediation"); seed=fungal_bioremediation_seed, width=1280, height=980)
  Main.FUNGAL_BIOREMEDIATION_VIGNETTE_BUNDLE[] = bundle
  bundle
end
...
If you rerun the seed cell above, the notebook tears down the previous local GUI server and starts a fresh Fungal Bioremediation session.
"""

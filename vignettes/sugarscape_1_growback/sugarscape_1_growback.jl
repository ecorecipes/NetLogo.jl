...
md"""
# Sugarscape 1 Growback vignette

This Pluto notebook wraps `eval/models/sugarscape_1_growback.jl` in the synthesized NetLogo-style interface used for the vignette rollout.
...
sugarscape_1_growback_seed = 1

# ╔═╡ 5e793407-fd26-4942-9e48-3fb75f835986
begin
  if !isdefined(Main, :SUGARSCAPE_1_GROWBACK_VIGNETTE_BUNDLE)
    const SUGARSCAPE_1_GROWBACK_VIGNETTE_BUNDLE = Ref{Any}(nothing)
  end
  previous = Main.SUGARSCAPE_1_GROWBACK_VIGNETTE_BUNDLE[]
  previous === nothing || NetLogoVignetteSupport.close_bundle!(previous)
  bundle = NetLogoVignetteSupport.benchmark_backend(Symbol("sugarscape_1_growback"); seed=sugarscape_1_growback_seed, width=1280, height=980)
  Main.SUGARSCAPE_1_GROWBACK_VIGNETTE_BUNDLE[] = bundle
  bundle
end
...
If you rerun the seed cell above, the notebook tears down the previous local GUI server and starts a fresh Sugarscape 1 Growback session.
"""

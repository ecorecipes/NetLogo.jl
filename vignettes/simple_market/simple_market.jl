...
md"""
# Simple Market vignette

This Pluto notebook wraps `eval/models/simple_market.jl` in the synthesized NetLogo-style interface used for the vignette rollout.
...
simple_market_seed = 1

# ╔═╡ 5e793407-fd26-4942-9e48-3fb75f835986
begin
  if !isdefined(Main, :SIMPLE_MARKET_VIGNETTE_BUNDLE)
    const SIMPLE_MARKET_VIGNETTE_BUNDLE = Ref{Any}(nothing)
  end
  previous = Main.SIMPLE_MARKET_VIGNETTE_BUNDLE[]
  previous === nothing || NetLogoVignetteSupport.close_bundle!(previous)
  bundle = NetLogoVignetteSupport.benchmark_backend(Symbol("simple_market"); seed=simple_market_seed, width=1280, height=980)
  Main.SIMPLE_MARKET_VIGNETTE_BUNDLE[] = bundle
  bundle
end
...
If you rerun the seed cell above, the notebook tears down the previous local GUI server and starts a fresh Simple Market session.
"""

...
md"""
# DiffusionOfHIV vignette

This Pluto notebook wraps `eval/models/diffusion_of_hiv.jl` in the synthesized NetLogo-style interface used for the vignette rollout.
...
diffusion_of_hiv_seed = 1

# ╔═╡ 5e793407-fd26-4942-9e48-3fb75f835986
begin
  if !isdefined(Main, :DIFFUSION_OF_HIV_VIGNETTE_BUNDLE)
    const DIFFUSION_OF_HIV_VIGNETTE_BUNDLE = Ref{Any}(nothing)
  end
  previous = Main.DIFFUSION_OF_HIV_VIGNETTE_BUNDLE[]
  previous === nothing || NetLogoVignetteSupport.close_bundle!(previous)
  bundle = NetLogoVignetteSupport.benchmark_backend(Symbol("diffusion_of_hiv"); seed=diffusion_of_hiv_seed, width=1280, height=980)
  Main.DIFFUSION_OF_HIV_VIGNETTE_BUNDLE[] = bundle
  bundle
end
...
If you rerun the seed cell above, the notebook tears down the previous local GUI server and starts a fresh DiffusionOfHIV session.
"""

...
md"""
# Public Good vignette

This Pluto notebook wraps `eval/models/public_good.jl` in the synthesized NetLogo-style interface used for the vignette rollout.
...
begin
  if !isdefined(Main, :PUBLIC_GOOD_VIGNETTE_BUNDLE)
    const PUBLIC_GOOD_VIGNETTE_BUNDLE = Ref{Any}(nothing)
  end
  previous = Main.PUBLIC_GOOD_VIGNETTE_BUNDLE[]
  previous === nothing || NetLogoVignetteSupport.close_bundle!(previous)
  bundle = NetLogoVignetteSupport.benchmark_backend(Symbol("public_good"); seed=public_good_seed, width=1280, height=980)
  Main.PUBLIC_GOOD_VIGNETTE_BUNDLE[] = bundle
  bundle
end
...
If you rerun the seed cell above, the notebook tears down the previous local GUI server and starts a fresh Public Good session.
"""

...
md"""
# BombScenario vignette

This Pluto notebook wraps `eval/models/bomb_scenario.jl` in the synthesized NetLogo-style interface used for the vignette rollout.
...
bomb_scenario_seed = 1

# ╔═╡ 5e793407-fd26-4942-9e48-3fb75f835986
begin
  if !isdefined(Main, :BOMB_SCENARIO_VIGNETTE_BUNDLE)
    const BOMB_SCENARIO_VIGNETTE_BUNDLE = Ref{Any}(nothing)
  end
  previous = Main.BOMB_SCENARIO_VIGNETTE_BUNDLE[]
  previous === nothing || NetLogoVignetteSupport.close_bundle!(previous)
  bundle = NetLogoVignetteSupport.benchmark_backend(Symbol("bomb_scenario"); seed=bomb_scenario_seed, width=1280, height=980)
  Main.BOMB_SCENARIO_VIGNETTE_BUNDLE[] = bundle
  bundle
end
...
If you rerun the seed cell above, the notebook tears down the previous local GUI server and starts a fresh BombScenario session.
"""

...
md"""
# Traffic Grid vignette

This Pluto notebook wraps `eval/models/traffic_grid.jl` in the synthesized NetLogo-style interface used for the vignette rollout.
...
begin
  if !isdefined(Main, :TRAFFIC_GRID_VIGNETTE_BUNDLE)
    const TRAFFIC_GRID_VIGNETTE_BUNDLE = Ref{Any}(nothing)
  end
  previous = Main.TRAFFIC_GRID_VIGNETTE_BUNDLE[]
  previous === nothing || NetLogoVignetteSupport.close_bundle!(previous)
  bundle = NetLogoVignetteSupport.benchmark_backend(Symbol("traffic_grid"); seed=traffic_grid_seed, width=1280, height=980)
  Main.TRAFFIC_GRID_VIGNETTE_BUNDLE[] = bundle
  bundle
end
...
If you rerun the seed cell above, the notebook tears down the previous local GUI server and starts a fresh Traffic Grid session.
"""

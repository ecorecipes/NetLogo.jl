...
md"""
# Contagion SI vignette

This Pluto notebook wraps `eval/models/contagion_si.jl` in the synthesized NetLogo-style interface used for the vignette rollout.
...
begin
  if !isdefined(Main, :CONTAGION_SI_VIGNETTE_BUNDLE)
    const CONTAGION_SI_VIGNETTE_BUNDLE = Ref{Any}(nothing)
  end
  previous = Main.CONTAGION_SI_VIGNETTE_BUNDLE[]
  previous === nothing || NetLogoVignetteSupport.close_bundle!(previous)
  bundle = NetLogoVignetteSupport.benchmark_backend(Symbol("contagion_si"); seed=contagion_si_seed, width=1280, height=980)
  Main.CONTAGION_SI_VIGNETTE_BUNDLE[] = bundle
  bundle
end
...
If you rerun the seed cell above, the notebook tears down the previous local GUI server and starts a fresh Contagion SI session.
"""

...
This Pluto notebook wraps `eval/models/recruiting.jl` in the synthesized NetLogo-style interface used for the vignette rollout.
...
begin
  if !isdefined(Main, :RECRUITING_VIGNETTE_BUNDLE)
    const RECRUITING_VIGNETTE_BUNDLE = Ref{Any}(nothing)
  end
  previous = Main.RECRUITING_VIGNETTE_BUNDLE[]
  previous === nothing || NetLogoVignetteSupport.close_bundle!(previous)
  bundle = NetLogoVignetteSupport.benchmark_backend(Symbol("recruiting"); seed=recruiting_seed, width=1280, height=980)
  Main.RECRUITING_VIGNETTE_BUNDLE[] = bundle
  bundle
end

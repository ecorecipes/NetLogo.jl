...
md"""
# Traffic Basic vignette

This Pluto notebook wraps `eval/models/traffic_basic.jl` in the synthesized NetLogo-style interface used for the vignette rollout.
...
traffic_basic_seed = 1

# ╔═╡ 5e793407-fd26-4942-9e48-3fb75f835986
begin
  if !isdefined(Main, :TRAFFIC_BASIC_VIGNETTE_BUNDLE)
    const TRAFFIC_BASIC_VIGNETTE_BUNDLE = Ref{Any}(nothing)
  end
  previous = Main.TRAFFIC_BASIC_VIGNETTE_BUNDLE[]
  previous === nothing || NetLogoVignetteSupport.close_bundle!(previous)
  bundle = NetLogoVignetteSupport.benchmark_backend(Symbol("traffic_basic"); seed=traffic_basic_seed, width=1280, height=980)
  Main.TRAFFIC_BASIC_VIGNETTE_BUNDLE[] = bundle
  bundle
end
...
If you rerun the seed cell above, the notebook tears down the previous local GUI server and starts a fresh Traffic Basic session.
"""

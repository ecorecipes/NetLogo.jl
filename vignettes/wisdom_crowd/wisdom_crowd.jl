...
md"""
# Wisdom of Crowd vignette

This Pluto notebook wraps `eval/models/wisdom_crowd.jl` in the synthesized NetLogo-style interface used for the vignette rollout.
...
begin
  if !isdefined(Main, :WISDOM_CROWD_VIGNETTE_BUNDLE)
    const WISDOM_CROWD_VIGNETTE_BUNDLE = Ref{Any}(nothing)
  end
  previous = Main.WISDOM_CROWD_VIGNETTE_BUNDLE[]
  previous === nothing || NetLogoVignetteSupport.close_bundle!(previous)
  bundle = NetLogoVignetteSupport.benchmark_backend(Symbol("wisdom_crowd"); seed=wisdom_crowd_seed, width=1280, height=980)
  Main.WISDOM_CROWD_VIGNETTE_BUNDLE[] = bundle
  bundle
end
...
If you rerun the seed cell above, the notebook tears down the previous local GUI server and starts a fresh Wisdom of Crowd session.
"""

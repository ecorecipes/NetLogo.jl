...
md"""
# Drug Use Overdose vignette

This Pluto notebook wraps `eval/models/drug_use_overdose.jl` in the synthesized NetLogo-style interface used for the vignette rollout.
...
drug_use_overdose_seed = 1

# ╔═╡ 5e793407-fd26-4942-9e48-3fb75f835986
begin
  if !isdefined(Main, :DRUG_USE_OVERDOSE_VIGNETTE_BUNDLE)
    const DRUG_USE_OVERDOSE_VIGNETTE_BUNDLE = Ref{Any}(nothing)
  end
  previous = Main.DRUG_USE_OVERDOSE_VIGNETTE_BUNDLE[]
  previous === nothing || NetLogoVignetteSupport.close_bundle!(previous)
  bundle = NetLogoVignetteSupport.benchmark_backend(Symbol("drug_use_overdose"); seed=drug_use_overdose_seed, width=1280, height=980)
  Main.DRUG_USE_OVERDOSE_VIGNETTE_BUNDLE[] = bundle
  bundle
end
...
If you rerun the seed cell above, the notebook tears down the previous local GUI server and starts a fresh Drug Use Overdose session.
"""

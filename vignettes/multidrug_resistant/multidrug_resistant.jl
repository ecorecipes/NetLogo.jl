...
md"""
# MultidrugResistant vignette

This Pluto notebook wraps `eval/models/multidrug_resistant.jl` in the synthesized NetLogo-style interface used for the vignette rollout.
...
multidrug_resistant_seed = 1

# ╔═╡ 5e793407-fd26-4942-9e48-3fb75f835986
begin
  if !isdefined(Main, :MULTIDRUG_RESISTANT_VIGNETTE_BUNDLE)
    const MULTIDRUG_RESISTANT_VIGNETTE_BUNDLE = Ref{Any}(nothing)
  end
  previous = Main.MULTIDRUG_RESISTANT_VIGNETTE_BUNDLE[]
  previous === nothing || NetLogoVignetteSupport.close_bundle!(previous)
  bundle = NetLogoVignetteSupport.benchmark_backend(Symbol("multidrug_resistant"); seed=multidrug_resistant_seed, width=1280, height=980)
  Main.MULTIDRUG_RESISTANT_VIGNETTE_BUNDLE[] = bundle
  bundle
end
...
If you rerun the seed cell above, the notebook tears down the previous local GUI server and starts a fresh MultidrugResistant session.
"""

...
md"""
# Wealth Distribution vignette

This Pluto notebook wraps `eval/models/wealth_distribution.jl` in the synthesized NetLogo-style interface used for the vignette rollout.
...
wealth_distribution_seed = 1

# ╔═╡ 5e793407-fd26-4942-9e48-3fb75f835986
begin
  if !isdefined(Main, :WEALTH_DISTRIBUTION_VIGNETTE_BUNDLE)
    const WEALTH_DISTRIBUTION_VIGNETTE_BUNDLE = Ref{Any}(nothing)
  end
  previous = Main.WEALTH_DISTRIBUTION_VIGNETTE_BUNDLE[]
  previous === nothing || NetLogoVignetteSupport.close_bundle!(previous)
  bundle = NetLogoVignetteSupport.benchmark_backend(Symbol("wealth_distribution"); seed=wealth_distribution_seed, width=1280, height=980)
  Main.WEALTH_DISTRIBUTION_VIGNETTE_BUNDLE[] = bundle
  bundle
end
...
If you rerun the seed cell above, the notebook tears down the previous local GUI server and starts a fresh Wealth Distribution session.
"""

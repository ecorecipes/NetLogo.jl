...
md"""
# CA 1D Elementary vignette

This Pluto notebook wraps `eval/models/ca_1d_elementary.jl` in the synthesized NetLogo-style interface used for the vignette rollout.
...
ca_1d_elementary_seed = 1

# ╔═╡ 5e793407-fd26-4942-9e48-3fb75f835986
begin
  if !isdefined(Main, :CA_1D_ELEMENTARY_VIGNETTE_BUNDLE)
    const CA_1D_ELEMENTARY_VIGNETTE_BUNDLE = Ref{Any}(nothing)
  end
  previous = Main.CA_1D_ELEMENTARY_VIGNETTE_BUNDLE[]
  previous === nothing || NetLogoVignetteSupport.close_bundle!(previous)
  bundle = NetLogoVignetteSupport.benchmark_backend(Symbol("ca_1d_elementary"); seed=ca_1d_elementary_seed, width=1280, height=980)
  Main.CA_1D_ELEMENTARY_VIGNETTE_BUNDLE[] = bundle
  bundle
end
...
If you rerun the seed cell above, the notebook tears down the previous local GUI server and starts a fresh CA 1D Elementary session.
"""

...
md"""
# COIN Learning vignette

This Pluto notebook wraps `eval/models/coin.jl` in the synthesized NetLogo-style interface used for the vignette rollout.
...
begin
  if !isdefined(Main, :COIN_VIGNETTE_BUNDLE)
    const COIN_VIGNETTE_BUNDLE = Ref{Any}(nothing)
  end
  previous = Main.COIN_VIGNETTE_BUNDLE[]
  previous === nothing || NetLogoVignetteSupport.close_bundle!(previous)
  bundle = NetLogoVignetteSupport.benchmark_backend(Symbol("coin"); seed=coin_seed, width=1280, height=980)
  Main.COIN_VIGNETTE_BUNDLE[] = bundle
  bundle
end
...
If you rerun the seed cell above, the notebook tears down the previous local GUI server and starts a fresh COIN Learning session.
"""

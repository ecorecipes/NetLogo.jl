...
md"""
# Forest Dewdney vignette

This Pluto notebook wraps `eval/models/forest_dewdney.jl` in the synthesized NetLogo-style interface used for the vignette rollout.
...
forest_dewdney_seed = 1

# ╔═╡ 5e793407-fd26-4942-9e48-3fb75f835986
begin
  if !isdefined(Main, :FOREST_DEWDNEY_VIGNETTE_BUNDLE)
    const FOREST_DEWDNEY_VIGNETTE_BUNDLE = Ref{Any}(nothing)
  end
  previous = Main.FOREST_DEWDNEY_VIGNETTE_BUNDLE[]
  previous === nothing || NetLogoVignetteSupport.close_bundle!(previous)
  bundle = NetLogoVignetteSupport.benchmark_backend(Symbol("forest_dewdney"); seed=forest_dewdney_seed, width=1280, height=980)
  Main.FOREST_DEWDNEY_VIGNETTE_BUNDLE[] = bundle
  bundle
end
...
If you rerun the seed cell above, the notebook tears down the previous local GUI server and starts a fresh Forest Dewdney session.
"""

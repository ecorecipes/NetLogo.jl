...
md"""
# Team Assembly vignette

This Pluto notebook wraps `eval/models/team_assembly.jl` in the synthesized NetLogo-style interface used for the vignette rollout.
...
team_assembly_seed = 1

# ╔═╡ 5e793407-fd26-4942-9e48-3fb75f835986
begin
  if !isdefined(Main, :TEAM_ASSEMBLY_VIGNETTE_BUNDLE)
    const TEAM_ASSEMBLY_VIGNETTE_BUNDLE = Ref{Any}(nothing)
  end
  previous = Main.TEAM_ASSEMBLY_VIGNETTE_BUNDLE[]
  previous === nothing || NetLogoVignetteSupport.close_bundle!(previous)
  bundle = NetLogoVignetteSupport.benchmark_backend(Symbol("team_assembly"); seed=team_assembly_seed, width=1280, height=980)
  Main.TEAM_ASSEMBLY_VIGNETTE_BUNDLE[] = bundle
  bundle
end
...
If you rerun the seed cell above, the notebook tears down the previous local GUI server and starts a fresh Team Assembly session.
"""

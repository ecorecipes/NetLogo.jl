...
md"""
# CSV Network Foraging vignette

This Pluto notebook wraps `eval/models/csv_network_foraging.jl` in the synthesized NetLogo-style interface used for the vignette rollout.
...
csv_network_foraging_seed = 1

# ╔═╡ 5e793407-fd26-4942-9e48-3fb75f835986
begin
  if !isdefined(Main, :CSV_NETWORK_FORAGING_VIGNETTE_BUNDLE)
    const CSV_NETWORK_FORAGING_VIGNETTE_BUNDLE = Ref{Any}(nothing)
  end
  previous = Main.CSV_NETWORK_FORAGING_VIGNETTE_BUNDLE[]
  previous === nothing || NetLogoVignetteSupport.close_bundle!(previous)
  bundle = NetLogoVignetteSupport.benchmark_backend(Symbol("csv_network_foraging"); seed=csv_network_foraging_seed, width=1280, height=980)
  Main.CSV_NETWORK_FORAGING_VIGNETTE_BUNDLE[] = bundle
  bundle
end
...
If you rerun the seed cell above, the notebook tears down the previous local GUI server and starts a fresh CSV Network Foraging session.
"""

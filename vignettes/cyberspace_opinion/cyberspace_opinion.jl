...
md"""
# Cyberspace Opinion vignette

This Pluto notebook wraps `eval/models/cyberspace_opinion.jl` in the synthesized NetLogo-style interface used for the vignette rollout.
...
cyberspace_opinion_seed = 1

# ╔═╡ 5e793407-fd26-4942-9e48-3fb75f835986
begin
  if !isdefined(Main, :CYBERSPACE_OPINION_VIGNETTE_BUNDLE)
    const CYBERSPACE_OPINION_VIGNETTE_BUNDLE = Ref{Any}(nothing)
  end
  previous = Main.CYBERSPACE_OPINION_VIGNETTE_BUNDLE[]
  previous === nothing || NetLogoVignetteSupport.close_bundle!(previous)
  bundle = NetLogoVignetteSupport.benchmark_backend(Symbol("cyberspace_opinion"); seed=cyberspace_opinion_seed, width=1280, height=980)
  Main.CYBERSPACE_OPINION_VIGNETTE_BUNDLE[] = bundle
  bundle
end
...
If you rerun the seed cell above, the notebook tears down the previous local GUI server and starts a fresh Cyberspace Opinion session.
"""

...
md"""
# Contact Process 2D vignette

This Pluto notebook wraps `eval/models/contactprocess.jl` in the synthesized NetLogo-style interface used for the vignette rollout.
...
contactprocess_seed = 1

# ╔═╡ 5e793407-fd26-4942-9e48-3fb75f835986
begin
  if !isdefined(Main, :CONTACTPROCESS_VIGNETTE_BUNDLE)
    const CONTACTPROCESS_VIGNETTE_BUNDLE = Ref{Any}(nothing)
  end
  previous = Main.CONTACTPROCESS_VIGNETTE_BUNDLE[]
  previous === nothing || NetLogoVignetteSupport.close_bundle!(previous)
  bundle = NetLogoVignetteSupport.benchmark_backend(Symbol("contactprocess"); seed=contactprocess_seed, width=1280, height=980)
  Main.CONTACTPROCESS_VIGNETTE_BUNDLE[] = bundle
  bundle
end
...
If you rerun the seed cell above, the notebook tears down the previous local GUI server and starts a fresh Contact Process 2D session.
"""

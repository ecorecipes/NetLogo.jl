...
md"""
# Small Worlds vignette

This Pluto notebook wraps `eval/models/small_worlds.jl` in the synthesized NetLogo-style interface used for the vignette rollout.
...
begin
  if !isdefined(Main, :SMALL_WORLDS_VIGNETTE_BUNDLE)
    const SMALL_WORLDS_VIGNETTE_BUNDLE = Ref{Any}(nothing)
  end
  previous = Main.SMALL_WORLDS_VIGNETTE_BUNDLE[]
  previous === nothing || NetLogoVignetteSupport.close_bundle!(previous)
  bundle = NetLogoVignetteSupport.benchmark_backend(Symbol("small_worlds"); seed=small_worlds_seed, width=1280, height=980)
  Main.SMALL_WORLDS_VIGNETTE_BUNDLE[] = bundle
  bundle
end
...
If you rerun the seed cell above, the notebook tears down the previous local GUI server and starts a fresh Small Worlds session.
"""

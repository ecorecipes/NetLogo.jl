...
md"""
# Percolation vignette

This Pluto notebook wraps `eval/models/percolation.jl` in the synthesized NetLogo-style interface used for the vignette rollout.
...
begin
  if !isdefined(Main, :PERCOLATION_VIGNETTE_BUNDLE)
    const PERCOLATION_VIGNETTE_BUNDLE = Ref{Any}(nothing)
  end
  previous = Main.PERCOLATION_VIGNETTE_BUNDLE[]
  previous === nothing || NetLogoVignetteSupport.close_bundle!(previous)
  bundle = NetLogoVignetteSupport.benchmark_backend(Symbol("percolation"); seed=percolation_seed, width=1280, height=980)
  Main.PERCOLATION_VIGNETTE_BUNDLE[] = bundle
  bundle
end
...
If you rerun the seed cell above, the notebook tears down the previous local GUI server and starts a fresh Percolation session.
"""

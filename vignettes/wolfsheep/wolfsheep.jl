...
md"""
# Wolf Sheep Predation vignette

This Pluto notebook wraps `eval/models/wolfsheep.jl` in the synthesized NetLogo-style interface used for the vignette rollout.
...
begin
  if !isdefined(Main, :WOLFSHEEP_VIGNETTE_BUNDLE)
    const WOLFSHEEP_VIGNETTE_BUNDLE = Ref{Any}(nothing)
  end
  previous = Main.WOLFSHEEP_VIGNETTE_BUNDLE[]
  previous === nothing || NetLogoVignetteSupport.close_bundle!(previous)
  bundle = NetLogoVignetteSupport.benchmark_backend(Symbol("wolfsheep"); seed=wolfsheep_seed, width=1280, height=980)
  Main.WOLFSHEEP_VIGNETTE_BUNDLE[] = bundle
  bundle
end
...
If you rerun the seed cell above, the notebook tears down the previous local GUI server and starts a fresh Wolf Sheep Predation session.
"""

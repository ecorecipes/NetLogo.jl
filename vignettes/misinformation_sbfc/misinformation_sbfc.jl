...
md"""
# Misinformation SBFC vignette

This Pluto notebook wraps `eval/models/misinformation_sbfc.jl` in the synthesized NetLogo-style interface used for the vignette rollout.
...
misinformation_sbfc_seed = 1

# ╔═╡ 5e793407-fd26-4942-9e48-3fb75f835986
begin
  if !isdefined(Main, :MISINFORMATION_SBFC_VIGNETTE_BUNDLE)
    const MISINFORMATION_SBFC_VIGNETTE_BUNDLE = Ref{Any}(nothing)
  end
  previous = Main.MISINFORMATION_SBFC_VIGNETTE_BUNDLE[]
  previous === nothing || NetLogoVignetteSupport.close_bundle!(previous)
  bundle = NetLogoVignetteSupport.benchmark_backend(Symbol("misinformation_sbfc"); seed=misinformation_sbfc_seed, width=1280, height=980)
  Main.MISINFORMATION_SBFC_VIGNETTE_BUNDLE[] = bundle
  bundle
end
...
If you rerun the seed cell above, the notebook tears down the previous local GUI server and starts a fresh Misinformation SBFC session.
"""

...
md"""
# Rational Evolving PD vignette

This Pluto notebook wraps `eval/models/rational_evolving_pd.jl` in the synthesized NetLogo-style interface used for the vignette rollout.
...
rational_evolving_pd_seed = 1

# ╔═╡ 5e793407-fd26-4942-9e48-3fb75f835986
begin
  if !isdefined(Main, :RATIONAL_EVOLVING_PD_VIGNETTE_BUNDLE)
    const RATIONAL_EVOLVING_PD_VIGNETTE_BUNDLE = Ref{Any}(nothing)
  end
  previous = Main.RATIONAL_EVOLVING_PD_VIGNETTE_BUNDLE[]
  previous === nothing || NetLogoVignetteSupport.close_bundle!(previous)
  bundle = NetLogoVignetteSupport.benchmark_backend(Symbol("rational_evolving_pd"); seed=rational_evolving_pd_seed, width=1280, height=980)
  Main.RATIONAL_EVOLVING_PD_VIGNETTE_BUNDLE[] = bundle
  bundle
end
...
If you rerun the seed cell above, the notebook tears down the previous local GUI server and starts a fresh Rational Evolving PD session.
"""

...
md"""
# Infinite Life vignette

This Pluto notebook wraps `eval/models/infinite_life.jl` in the synthesized NetLogo-style interface used for the vignette rollout.
...
infinite_life_seed = 1

# ╔═╡ 5e793407-fd26-4942-9e48-3fb75f835986
begin
  if !isdefined(Main, :INFINITE_LIFE_VIGNETTE_BUNDLE)
    const INFINITE_LIFE_VIGNETTE_BUNDLE = Ref{Any}(nothing)
  end
  previous = Main.INFINITE_LIFE_VIGNETTE_BUNDLE[]
  previous === nothing || NetLogoVignetteSupport.close_bundle!(previous)
  bundle = NetLogoVignetteSupport.benchmark_backend(Symbol("infinite_life"); seed=infinite_life_seed, width=1280, height=980)
  Main.INFINITE_LIFE_VIGNETTE_BUNDLE[] = bundle
  bundle
end
...
If you rerun the seed cell above, the notebook tears down the previous local GUI server and starts a fresh Infinite Life session.
"""

...
md"""
# Preferential Attachment vignette

This Pluto notebook wraps `eval/models/preferential_attachment.jl` in the synthesized NetLogo-style interface used for the vignette rollout.
...
preferential_attachment_seed = 1

# ╔═╡ 5e793407-fd26-4942-9e48-3fb75f835986
begin
  if !isdefined(Main, :PREFERENTIAL_ATTACHMENT_VIGNETTE_BUNDLE)
    const PREFERENTIAL_ATTACHMENT_VIGNETTE_BUNDLE = Ref{Any}(nothing)
  end
  previous = Main.PREFERENTIAL_ATTACHMENT_VIGNETTE_BUNDLE[]
  previous === nothing || NetLogoVignetteSupport.close_bundle!(previous)
  bundle = NetLogoVignetteSupport.benchmark_backend(Symbol("preferential_attachment"); seed=preferential_attachment_seed, width=1280, height=980)
  Main.PREFERENTIAL_ATTACHMENT_VIGNETTE_BUNDLE[] = bundle
  bundle
end
...
If you rerun the seed cell above, the notebook tears down the previous local GUI server and starts a fresh Preferential Attachment session.
"""

...
md"""
# Simple Birth Rates vignette

This Pluto notebook wraps `eval/models/simple_birth_rates.jl` in the synthesized NetLogo-style interface used for the vignette rollout.
...
simple_birth_rates_seed = 1

# ╔═╡ 5e793407-fd26-4942-9e48-3fb75f835986
begin
  if !isdefined(Main, :SIMPLE_BIRTH_RATES_VIGNETTE_BUNDLE)
    const SIMPLE_BIRTH_RATES_VIGNETTE_BUNDLE = Ref{Any}(nothing)
  end
  previous = Main.SIMPLE_BIRTH_RATES_VIGNETTE_BUNDLE[]
  previous === nothing || NetLogoVignetteSupport.close_bundle!(previous)
  bundle = NetLogoVignetteSupport.benchmark_backend(Symbol("simple_birth_rates"); seed=simple_birth_rates_seed, width=1280, height=980)
  Main.SIMPLE_BIRTH_RATES_VIGNETTE_BUNDLE[] = bundle
  bundle
end
...
If you rerun the seed cell above, the notebook tears down the previous local GUI server and starts a fresh Simple Birth Rates session.
"""
